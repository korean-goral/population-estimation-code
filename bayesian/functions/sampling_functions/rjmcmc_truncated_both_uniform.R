...
rjmcmc_truncated_both_uniform <- function(n, X, N_prior, n_init, sigma_prior, niters, xlims, ylims, burn_in = 10000, init_delta = 10, init_tune=c(0.1, 0.1, 2), N_updates = 15, log, stochastic = TRUE, lam0_prior = FALSE, check_point = FALSE, monitorS=FALSE)
{
  ...
  for(iter in 1:niters) {
    ...
    for (i in seq(1:N_updates)){
      ...
      if((N_prior[1] < N.cand) & (N.cand <= N_prior[2])) { # truncated negbin prior.
